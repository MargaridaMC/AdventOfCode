...
def parse_input(data):
    ...
    available_towel_patterns = available_towel_patterns.split(", ")
    desired_designs = desired_designs.splitlines()
    return available_towel_patterns, desired_designs

@cache
def count_possible_towel_combination(desired_design, break_on_first_match=False):
    ...
    for pattern in available_towel_patterns:
        if desired_design.startswith(pattern):
            temp = count_possible_towel_combination(desired_design[len(pattern):], break_on_first_match)
...
def part2(data):
    ...
    possible_design_count = 0
    for desired_design in desired_designs:
        possible_design_count += count_possible_towel_combination(desired_design)
    return possible_design_count
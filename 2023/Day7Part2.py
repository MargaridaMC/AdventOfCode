...
def classify_hand(hand):
    ...
    if 'J' in hand and hand != 'JJJJJ':
        # Add Js to the letter with largest count
        # To avoid issues with the situation where j is the most frequent letter remove it from the count
        j_count = letter_count['J']
        del letter_count['J']
        letter_w_largest_count = max(letter_count, key=letter_count.get)
        letter_count[letter_w_largest_count] += j_count
...
def compare_hands_of_equal_type(hand1, hand2):
    ...
    LETTER_VALUE_MAPPING = {'J': -1, '2': 0, '3': 1, '4': 2, '5': 3, '6': 4, '7': 5, '8': 6, '9': 7, 'T': 8, 'Q': 10, 'K': 11, 'A': 12}

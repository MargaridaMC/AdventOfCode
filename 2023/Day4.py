...
# Part 2
cards_to_look_through = np.ones(len(input), dtype=int)
card_count = len(input)
while not (cards_to_look_through == 0).all():
    for card_id in range(1, len(input) + 1):
        n_copies = cards_to_look_through[card_id - 1]
        if n_copies == 0: continue
        cards_to_look_through[card_id - 1] = 0
        ...
        for j in range(1, n_matches + 1):
            cards_to_look_through[card_id + j - 1] += n_copies
